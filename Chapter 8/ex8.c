...
unsigned bfree (char *ptr, unsigned number_of_characters)
{
	Header *head_ptr;
	if (number_of_characters < sizeof (Header))
		return 0;
	head_ptr = (Header *) ptr;
	head_ptr -> s.size = number_of_characters / sizeof (Header);
	free ((void *) (head_ptr + 1));
	printf ("Freed block\n");
	return head_ptr -> s.size;
}
